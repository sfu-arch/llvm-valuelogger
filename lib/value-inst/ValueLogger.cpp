...
void InstruMemPass::visitReturnInst (ReturnInst &I) {
    ...
    for(auto aI = F->arg_begin(); aI != F->arg_end(); ++aI) {
        ...
        Value *args[] = {bc, ConstantInt::get(Type::getInt64Ty(I.getContext()), aI->getArgNo())
                        , ConstantInt::get(i64Ty, GetTypeEnum(argVal))};
...
int InstruMemPass::GetTypeEnum(Value* v) {
    if (v->getType()->isFloatTy()) return FLT;
    else if (v->getType()->isDoubleTy()) return DBL;
    else if (v->getType()->isIntegerTy(1)) return BOL;
    else if (v->getType()->isIntegerTy(8)) return CHR;
    else if (v->getType()->isIntegerTy(16)) return HLF;
    else if (v->getType()->isIntegerTy(32)) return WHL;
    else if (v->getType()->isIntegerTy(64)) return LNG;
    else if (v->getType()->isPointerTy()) return PTR;
